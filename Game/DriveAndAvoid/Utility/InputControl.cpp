...
void InputControl::Update()
{
	...
	{
		stick[0].x = (float)input_state.ThumbRX / (float)SHRT_MAX;
	}
	else
	{
		stick[0].x = ((float)input_state.ThumbRX / (float)SHRT_MIN);
	}
	if (input_state.ThumbLY > 0.0f)
	{
		stick[0].y = (float)input_state.ThumbRY / (float)SHRT_MAX;
	}
	else
	{
		stick[0].y = ((float)input_state.ThumbRY / (float)SHRT_MIN);
	}
}
